...
const productLayoutCard = (dataPane, { ...args }) => {
  ...
  <div class="shopping-card-title"><h4>${name}</h4></div>
  <div class="shopping-card-brand"><p>${brand}</p></div>
  <div class="shopping-card-price"><h4>Rs.${price}</h4></div></div></div>`);
};

//******************The below code are only for Modal purpose!***********************
/*************************************************************************************
 *************************************************************************************/
//Additional elements $modal
const productModalRender = ({ ...args }) => {
  ...
  <i id="cart_bag" data-name="${name}" class="fas fa-shopping-bag"></i>Add to Cart</button></div></div></div>
  </div>`);
};
...
const modalHandler = (data, modal) => {
  ...
  const quickShop = [
    ...document.querySelectorAll(".shopping-card-quickshop button")
  ];
